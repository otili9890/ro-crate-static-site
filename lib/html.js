  ...
  makePath(id) {
    if (id === this.crate.rootDataset["@id"]){
      return this.html.rootPage;
        ...
        return Path.join(this.html.path, "ro-crate-preview_files", id.replace(/\W/g, "") + ".html")
    }
  }
          ...
          if(val["@id"]) {
           ...
           if (target) {
             ...
             if (data[prop][i]?.name) {
              ...
              data[prop][i] = {
                "@path": Path.relative(Path.dirname(this.pagePath), this.makePath(val["@id"])),
                "@value" :  name || val["@id"] ,
